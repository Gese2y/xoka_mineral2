...
export class BankAccountPostingGroupComponent implements OnInit {
  ...
  getgebus() {
    ...
    this.bankAccountPostingGroupService.getgebus().subscribe(
      (responce) => {
        this.orgids = responce["procGenBusPostingGroupss"];
      
        console.log("getChartOfAccount ::", this.orgids);
      },
      (error) => {
        console.log("error");
      }
  ...
  registerBankAccountPostingGroup() {
   // this.bankAccountPostingGroup.orgid = this.workingUser.organization_code;
    this.bankAccountPostingGroup.branch=this.workingUser.organization_code;
