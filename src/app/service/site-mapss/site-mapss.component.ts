...
import { SiteMapssService } from "./site-mapss.service";
import { PopupService } from "src/app/pro-service/popup.service";
import { ServiceService } from '../service.service';
@Component({
  selector: 'app-site-mapss',
  templateUrl: './site-mapss.component.html',
  styleUrls: ['./site-mapss.component.css']
})
export class SiteMapssComponent implements OnInit {
...
constructor(private SiteMapssService: SiteMapssService,
...
public getCoordOnClick(event) {
  ...
  this.serviceService.gis_Plot_Id =  this.clickCoordinate
...
loadGeoJsonList() {
  (this.SiteMapssService as SiteMapssService).getListOfGeoJson().subscribe(
    (listOfGeoJson) => {
...
loadGeoJsonLayer(whichGeoJson) {
  this.SiteMapssService.getGeoJsonData(whichGeoJson).subscribe(
    (geoJsonData) => {
...
i = 0;
wfsCapabilities() {
  this.SiteMapssService.getCapabilities().subscribe((res: any) => {
...
async promiseWfsCapabilities() {
  console.log(
    ...
    await (this.SiteMapssService as any).promiseGetCapabilities()
  );
}

loadFeatures(typeName, outputFormat) {
  (this.SiteMapssService as any).getFeatures(typeName, outputFormat).subscribe(
    (feature) => {
