...
loadGeoJsonList() {
  (this.SiteMapService as SiteMapService).getListOfGeoJson().subscribe(
    (listOfGeoJson) => {
...
loadGeoJsonLayer(whichGeoJson) {
  this.SiteMapService.getGeoJsonData(whichGeoJson).subscribe(
    (geoJsonData) => {
...
i = 0;
wfsCapabilities() {
  this.SiteMapService.getCapabilities().subscribe((res: any) => {
...
async promiseWfsCapabilities() {
  console.log(
    ...
    await (this.SiteMapService as any).promiseGetCapabilities()
  );
}

loadFeatures(typeName, outputFormat) {
  (this.SiteMapService as any).getFeatures(typeName, outputFormat).subscribe(
    (feature) => {
