...
export class InventoryPostingSetupComponent implements OnInit {
  ...
  CheckPrimaryKey(){
    this._InventoryPostingSetupService.getcheckprimarykey(this.newInventoryPosting.store_Code.store_code).subscribe(
    (response)=>{
     // console.log(":: length",response.procInventoryPostingGroups.length);
      console.log(":: a ",this.newInventoryPosting.store_Code.store_code);
      console.log(":: res",response.stores.length);
    ...
    if(response.stores.length>0){
      ...
      const toast = this.notificationsService.warn(
        "Warning",
        "Store code is duplicate primary key please insert another value"
      );
    }
    },
    (error)=>{
...
this.newInventoryPosting.store_Code=this.newInventoryPosting.store_Code.store_code;
  ...
  searchChartOfAccount(event) {
    ...
    console.log("ff ::" + this.FilteredChartOfAccounts);
  }
  searchChartOfstore(event){
  
    this.FilteredChartOfStore = this.Store.filter((cc) =>
    cc.store_code.includes(event.query)
  );
  ...
  getStore() {
    this._InventoryPostingSetupService.getStore().subscribe(
      (stores) => {
        this.Store = stores;
