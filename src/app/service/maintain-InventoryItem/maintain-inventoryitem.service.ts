...
export class MaintainInventoryitemService {
  ...
  getgetStock_Bin_Detail() {
    return this.http.get(this.rootApiPath + "inventory/StockBinDetail");
  }
getcheckprimarykey(id){
   return this.http.get<any>(this.rootApiPath + "inventory/InventoryItems" + "/" +"InvID" +"/" + id);
}
  getClasses() {
    return this.http.get(this.rootApiPath + "finance/cItemClass");
  }
  getstockingUnit() {
    return this.http.get(this.rootApiPath + "finance/cUnit");
  }
  getVocher_No() {
    return this.http.get(this.rootApiPath + "inventory/StockBinDetail");
  }
